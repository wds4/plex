...
export default class AllWordsTable extends React.Component {
    ...
    componentDidMount() {
        ...
        for (var w=0;w<aWords.length;w++) {
            ...
            var nextWord_wordTypes = "";
            for (var t=0;t<aWordTypes.length;t++) {
                var nextWordType = aWordTypes[t];
                nextWord_wordTypes += nextWordType;
                if (t+1 < aWordTypes.length) {
                    nextWord_wordTypes += ", "
                }
            }

            var nextWord_ipns = oNextWord.metaData.ipns;

            var nextWord_id = window.lookupSqlIDBySlug[nextWord_slug];
            var nextWord_name = "unknown";
            if (oNextWord.wordData.hasOwnProperty("name")) {
                nextWord_name = oNextWord.wordData.name;
            }

            var nextWord_button = "<div data-queryid="+w+" data-sqlid="+nextWord_id+" id=nextRowButton2_"+nextWord_id+" class='doSomethingButton_small nextRowEditButton2' style=margin-right:5px; >VIEW / EDIT</div>";
            var nextWord_id_html = nextWord_button + nextWord_id;
            ...
            var oWordData = {};
            oWordData.pathname = "/SingleWordGeneralInfo/"+nextWord_id;
            oWordData.wordsqlid = 'linkFrom_'+nextWord_id;
            oWordData.wordname = nextWord_name;
