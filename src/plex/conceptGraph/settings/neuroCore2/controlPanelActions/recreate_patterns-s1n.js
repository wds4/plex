...
const transferPatternFunctions = async () => {
    ...
    // var oSupersetPatterns = window.lookupWordBySlug["supersetFor_pattern"];
    var oSupersetPatterns = window.lookupWordBySlug["patterns_singleNode"];
    var aPatterns = oSupersetPatterns.globalDynamicData.specificInstances;
    for (var a=0;a<aPatterns.length;a++) {
      var nextPattern_slug = aPatterns[a];
      var oPattern = window.lookupWordBySlug[nextPattern_slug]
      var nextPattern_patternName = oPattern.patternData.name;
      var nextPattern_patternSlug = oPattern.patternData.slug;

      var nextPattern_patternName_converted = nextPattern_patternName.replaceAll(".","_").toLowerCase();

      var nextPattern_javascript = oPattern.patternData.singleNodeFieldsetData.javascript;
    ...
    electronFs.writeFile("src/plex/neuroCore2/neuroCoreFunctions/patterns/patterns-s1n.js", outputFile, (err) => {
        if (err)
            console.log(err);
        else {
