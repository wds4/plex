...
export const executeSingleAction = async (oAction,nc2CycleNumber,singlePatternNumber,singleActionNumber,whichNeuroCore) => {
    ...
    if (whichNeuroCore == "NeuroCore2") {
        ...
        var oWinNeuroCore = window.neuroCore
    }
    if (whichNeuroCore == "NeuroCore3") {
        var oRFL = MiscFunctions.cloneObj(window.ipfs.neuroCore.subject.oRFL)
        var oWinNeuroCore = window.ipfs.neuroCore
    }
        ...
        case "a-a-u1n-validation":
            ...
            try {
                ...
                for (var a=0;a<aParentJSONSchemaSequence.length;a++) {
                    ...
                    var aConceptGraphDefaultProperties = oWinNeuroCore.subject.oMainSchemaForConceptGraph.conceptGraphData.defaultPropertyDescriptors
            ...
            try {
                ...
                if (nF_pendingDeletion == false) {
                        ...
                        if ( (nF_propertyType=="string") || (nF_propertyType=="integer") || (nF_propertyType=="boolean") ) {
                            ...
                            oRFL.updated[nF_slug] = oNodeFrom;
                            oWinNeuroCore.oRFL.updated[nF_slug] = oNodeFrom;
                        }
                ...
                // oWinNeuroCore.oRFL.updated[nF_slug] = oNodeFrom;
                oRFL.updated[nT_slug] = oNodeTo;
                // oWinNeuroCore.oRFL.updated[nT_slug] = oNodeTo;
            } catch (err) {
...
oRFL.updated.mainSchemaForConceptGraph = oMainSchemaForConceptGraph;
        ...
        case "a-c-c1n-createjsonschema":
            ...
            try {
                ...
                oRFL.updated[node_slug] = oNode;
                // oWinNeuroCore.oRFL.updated[node_slug] = oNode;
...
// oWinNeuroCore.oRFL.updated[mainSchemaForConceptGraph_slug] = MiscFunctions.cloneObj(oMSFCG);
            } catch (err) {
...
oRFL.updated[nT_slug] = oNodeTo;
    ...
    for (var x=0;x<aNews.length;x++) {
        ...
        oWinNeuroCore.engine.oRecordOfUpdates[newUniqueIdentifier] = {}
        oWinNeuroCore.engine.oRecordOfUpdates[newUniqueIdentifier].old = {};
        oWinNeuroCore.engine.oRecordOfUpdates[newUniqueIdentifier].new = oWord_new;
        var infoHTML = "";
        infoHTML += "<div>";
        ...
        if (executeChanges=="yes") {
            ...
            if (oWinNeuroCore.subject.currentConceptGraphSqlID==window.currentConceptGraphSqlID) {
                window.lookupWordBySlug[nextUpdate_slug] = oWord_updated;
            }
            oWinNeuroCore.engine.changesMadeYetThisSupercycle = true;
            oWinNeuroCore.engine.changesMadeYetThisCycle = true;
            addPatternsToQueue(actionSlug,oWinNeuroCore);
        }
        var oAuxiliaryPatternData = {"searchMethod":"default"};
    ...
    for (var x=0;x<aUpdates.length;x++) {
        ...
        if (sWord_current != sWord_updated) {
            ...
            oWinNeuroCore.engine.oRecordOfUpdates[updateUniqueIdentifier] = {}
            oWinNeuroCore.engine.oRecordOfUpdates[updateUniqueIdentifier].old = MiscFunctions.cloneObj(oWord_current);
            oWinNeuroCore.engine.oRecordOfUpdates[updateUniqueIdentifier].new = MiscFunctions.cloneObj(oWord_updated);
            var infoHTML = "";
            infoHTML += "<div>";
    ...
    if (addToQueue) {
        ...
        if (auxAssistedQueue) {
            ...
            addAuxiliaryPatternDataToQueue(aAuxiliaryPatternData,whichNeuroCore,oWinNeuroCore);
            // 29 July 2022:
        ...
        // (via aPats = oWinNeuroCore.engine.oPatternsTriggeredByAction[actionSlug];)
        if (!auxAssistedQueue) {
            var aPats = oWinNeuroCore.engine.oPatternsTriggeredByAction[actionSlug];
            ...
            for (var p=0;p<aPats.length;p++) {
                ...
                var oPat = MiscFunctions.cloneObj(oWinNeuroCore.engine.oRFL.current[nextPat_slug]);
                ...
                if (!oWinNeuroCore.engine.oPatternsWithAuxiliaryDataQueue.hasOwnProperty(nextPat_slug)) {
                    oWinNeuroCore.engine.oPatternsWithAuxiliaryDataQueue[nextPat_slug] = [];
                }
                ...
                if (!isAPDP) {
                    oWinNeuroCore.engine.oPatternsWithAuxiliaryDataQueue[nextPat_slug].push(oAPD)
                }
            }
        }
    }
...
export const addAuxiliaryPatternDataToQueue = (aAuxiliaryPatternData,whichNeuroCore,oWinNeuroCore) => {
    for (var z=0;z<aAuxiliaryPatternData.length;z++) {
        ...
        var patt_wSlug = oWinNeuroCore.engine.oMapActionSlugToWordSlug[pattName];
        if (!oWinNeuroCore.engine.oPatternsWithAuxiliaryDataQueue.hasOwnProperty(patt_wSlug)) {
            oWinNeuroCore.engine.oPatternsWithAuxiliaryDataQueue[patt_wSlug] = [];
        }
        var isAPDP = isAPDPresent(patt_wSlug,oAPD,whichNeuroCore);
        if (!isAPDP) {
            oWinNeuroCore.engine.oPatternsWithAuxiliaryDataQueue[patt_wSlug].push(oAPD)
        }
    }
    var qHTML = "";
...
export const addPatternsWithAuxiliaryPatternDataToQueue = (actionSlug,oAuxiliaryPatternData) => {
    var action_wordSlug = oWinNeuroCore.engine.oMapPatternNameToWordSlug[actionSlug];
    var oAct = oWinNeuroCore.engine.oRFL.current[action_wordSlug];
    var sAuxiliaryPatternData = JSON.stringify(oAuxiliaryPatternData);
    if (oAct.actionData.hasOwnProperty("secondaryPatterns")) {
        ...
        for (var s=0;s<aSets.length;s++) {
            ...
            var oNextSet = oWinNeuroCore.engine.oRFL.current[nextSet_slug];
        ...
        for (var s=0;s<aIndividualPatterns.length;s++) {
            ...
            var nextPattern_wordSlug = oWinNeuroCore.engine.oMapActionSlugToWordSlug[nextPattern_patternName];
            var oNextPattern = oWinNeuroCore.engine.oRFL.current[nextPattern_wordSlug]
...
*/
// OLD METHOD
const addPatternsToQueue = (actionSlug,oWinNeuroCore) => {
    var action_wordSlug = oWinNeuroCore.engine.oMapPatternNameToWordSlug[actionSlug];
    var oAct = oWinNeuroCore.engine.oRFL.current[action_wordSlug];
