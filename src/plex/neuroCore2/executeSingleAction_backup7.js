...
const isAPDPresent = (nextPat,oAuxiliaryPatternData) => {
    ...
    var aAPDs = window.plexNeuroCore.patternsWithAuxiliaryDataQueue[nextPat]
...
export const executeSingleAction = async (oAction,plexNeuroCore,nc2CycleNumber,singlePatternNumber,singleActionNumber) => {
    ...
    switch (actionSlug) {
        case "a-a-u1n-01":
            ...
            try {
                ...
                oAuxiliaryPatternData = {};
                oAuxiliaryPatternData.searchMethod = "restrictedDomain";
                oAuxiliaryPatternData.patternName = "P.a.s1r.02";
                oAuxiliaryPatternData.inputField = "singleRelationship";
                oAuxiliaryPatternData.domains = {};
...
window.neuroCore.oRFL.updated[mainSchemaForConceptGraph_slug] = MiscFunctions.cloneObj(oMSFCG);
            ...
            try {
                ...
                if (!alreadyCreated) {
                    ...
                    if (!candidateRole5) {
                        ...
                        var newSubset_word_slug = "setFor_"+role4_concept_plural+"_organizedBy_"+role1_concept_singular + "_"+ newSubset_ipns.slice(-6);
                        ...
                        var newSubset_set_slug = role4_concept_plural+"_organizedBy_"+role1_concept_singular;
    ...
    for (var x=0;x<aNews.length;x++) {
        ...
        if (executeChanges=="yes") {
            ...
            window.plexNeuroCore.oRFL.updated[nextNew_slug] = oWord_new;
            window.plexNeuroCore.oRFL.new[nextNew_slug] = oWord_new;
            addPatternsToQueue(actionSlug,plexNeuroCore);
        }
        var oAuxiliaryPatternData = {"searchMethod":"default"};
        addPatternsWithAuxiliaryPatternDataToQueue(actionSlug,plexNeuroCore,oAuxiliaryPatternData);
    }
    ...
    for (var x=0;x<aUpdates.length;x++) {
        ...
        if (sWord_current != sWord_updated) {
            ...
            window.plexNeuroCore.oRecordOfUpdates[updateUniqueIdentifier].old = MiscFunctions.cloneObj(oWord_current);
            window.plexNeuroCore.oRecordOfUpdates[updateUniqueIdentifier].new = MiscFunctions.cloneObj(oWord_updated);
            var infoHTML = "";
            infoHTML += "<div>";
            ...
            if (executeChanges=="yes") {
                ...
                window.plexNeuroCore.oRFL.updated[nextUpdate_slug] = oWord_updated;
                addPatternsToQueue(actionSlug,plexNeuroCore);
            }
        }
    }
    if (addToQueue) {
        var aPats = window.plexNeuroCore.lookupPatternsTriggeredByAction[actionSlug];
        for (var p=0;p<aPats.length;p++) {
            ...
            if (!window.plexNeuroCore.patternsWithAuxiliaryDataQueue.hasOwnProperty(nextPat_slug)) {
                window.plexNeuroCore.patternsWithAuxiliaryDataQueue[nextPat_slug] = [];
            }
            ...
            if (!isAPDP) {
                window.plexNeuroCore.patternsWithAuxiliaryDataQueue[nextPat_slug].push(oAPD)
            }
        }
    }
...
const addPatternsWithAuxiliaryPatternDataToQueue = (actionSlug,plexNeuroCore,oAuxiliaryPatternData) => {
    var action_wordSlug = plexNeuroCore.oMapActionSlugToWordSlug[actionSlug];
    var oAct = window.lookupWordBySlug[action_wordSlug];
    var sAuxiliaryPatternData = JSON.stringify(oAuxiliaryPatternData);
    if (oAct.actionData.hasOwnProperty("secondaryPatterns")) {
        ...
        for (var s=0;s<aIndividualPatterns.length;s++) {
            ...
            var oNextPattern = window.lookupWordBySlug[nextPattern_wordSlug]
