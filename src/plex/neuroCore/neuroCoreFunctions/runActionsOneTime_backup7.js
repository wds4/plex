...
const runActionsOneTime = async (thisState) => {
    ...
    var oRawFileLookup_xyz = MiscFunctions.cloneObj(oActiveConceptGraph.rawFileLookup);
    // var oRawFileUpdatedLookup = oActiveConceptGraph.rawFileUpdatedLookup;
    var oPatterns_xyz = MiscFunctions.cloneObj(thisState.patterns);
    var oS1nPatternQueue = MiscFunctions.cloneObj(oPatterns_xyz.patternMatches.s1n);
    console.log("runActionsOneTime_oS1nPatternQueue: "+JSON.stringify(oS1nPatternQueue,null,4))
    var oS1rPatternQueue = MiscFunctions.cloneObj(oPatterns_xyz.patternMatches.s1r);
    var oS2rPatternQueue = MiscFunctions.cloneObj(oPatterns_xyz.patternMatches.s2r);

    var oMapPatternToActions = oPatterns_xyz.mapPatternToActions;
    // console.log("oMapPatternToActions: "+JSON.stringify(oMapPatternToActions,null,4));

    /*
    var aS1nPatternsRawSql = oPatterns_xyz.aS1nPatternsRawSql
    var aS1rPatternsRawSql = oPatterns_xyz.aS1rPatternsRawSql
    var aS2rPatternsRawSql = oPatterns_xyz.aS2rPatternsRawSql
    */
    ...
    if (isAllActionsChecked) {
        ...
        reportHTML += "<br>";
        reportHTML += "\\\\\\\\\\\\\<br>";
        reportHTML += await executeQueuedS1nActions(oS1nPatternQueue,oMapPatternToActions,oRawFileLookup_xyz)
        reportHTML += await executeQueuedS1rActions(oS1rPatternQueue,oMapPatternToActions,oRawFileLookup_xyz)
        reportHTML += await executeQueuedS2rActions(oS2rPatternQueue,oMapPatternToActions,oRawFileLookup_xyz)
    }
