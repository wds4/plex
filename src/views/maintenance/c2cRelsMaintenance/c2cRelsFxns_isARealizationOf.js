...
// loof for user-entered rel: (nF) - isARealizationOf - (nT)
export const generateInitialPatternList = (words_in_obj) => {
    console.log("generateInitialPatternList isARealizationOf")
    var output_obj = {};
    output_obj.patternInfo = {};
    output_obj.patternInfo.description = "(nF) - isARealizationOf - (nT); nF and nT are both concepts";
    output_obj.patternInfo.action = {};
    output_obj.patternInfo.action.description = "create rel: (nF:wordType) - isASpecificInstanceOf - (nT:superset) and place in schemaFor[nT]";
    ...
    for (const [nextWord_slug, nextWord_rF_obj] of Object.entries(words_in_obj)) {
        ...
        if (jQuery.inArray("schema",nextWord_wordTypes_arr) > -1) {
            ...
            for (var r=0;r<numRels;r++) {
                ...
                if (nextRel_rT_slug=="isARealizationOf") {
                    ...
                    // nextRel_obj.relationshipType.isARealizationOfData.setMappings
                    // if isARealizationOfData does not exist, assume one relationship such that
                    // nextRel_obj.relationshipType.isARealizationOfData.setMappings[0] = (superset of nT)

                    // If isARealizationOfData is absent, create it prior to the next processing step.
                    if (!nextRel_obj.relationshipType.hasOwnProperty("isARealizationOfData")) {
                        nextRel_obj.relationshipType.isARealizationOfData = {}
                        nextRel_obj.relationshipType.isARealizationOfData.setMappings = []
                        nextRel_obj.relationshipType.isARealizationOfData.setMappings.push(nT_concept_rF_obj.conceptData.nodes.superset.slug);
                    }
                    ...
                    // var nextRel_str = JSON.stringify(nextRel_obj,null,4);
                    // console.log("isARealizationOf nextRel_str+ "+nextRel_str)
                    var numSetMappings = nextRel_obj.relationshipType.isARealizationOfData.setMappings.length;
                    // console.log("isARealizationOf numSetMappings: "+numSetMappings)
                    for (var m=0;m<numSetMappings;m++) {
                        var nextSetMapping_slug = nextRel_obj.relationshipType.isARealizationOfData.setMappings[m];
                        ...
                        relToAdd_obj.nodeFrom.slug = nF_concept_rF_obj.conceptData.nodes.wordType.slug;
                        relToAdd_obj.nodeTo.slug = nextSetMapping_slug;
...
export const determineStatusOfActionPatterns = (words_in_obj,reports_isARealizationOf_in_obj) => {
    console.log("flagActionsAsActionable isARealizationOf")
    var reports_isARealizationOf_out_obj = MiscFunctions.cloneObj(reports_isARealizationOf_in_obj);
    ...
    var completed_tot = 0;
    var patternsList_arr = reports_isARealizationOf_out_obj.patternsList;
    ...
    reports_isARealizationOf_out_obj.numPatterns.actionable = actionable_tot;
    reports_isARealizationOf_out_obj.numPatterns.notActionable = completed_tot;
    reports_isARealizationOf_out_obj.numPatterns.dunnoYetIfActionable = 0;

    return reports_isARealizationOf_out_obj;
}

export const makeChanges = async (words_in_obj,reports_obj) => {
    var words_out_obj = MiscFunctions.cloneObj(words_in_obj);
    var numPatterns = reports_obj.isARealizationOf.patternsList.length;
    for (var p=0;p<numPatterns;p++) {
        var nextPattern_obj = reports_obj.isARealizationOf.patternsList[p]
        if (nextPattern_obj.actionable) {
            console.log("actionable: isARealizationOf pattern: "+p)
