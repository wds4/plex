...
import { gDDPatterns, gDDActions } from './cp_valenceTransfers.js';
import { removeDuplicatesFromGlobalDynamicData } from '../../../addANewConcept.js';
import sendAsync from '../../../../renderer';
import * as ControlPanelFunctions from '../maintenanceControlPanel.js';
const jQuery = require("jquery");

// coda for this pattern-match-function group: gDD (was XYZ)

export var words_gDD_pre_rF_obj = {};
export var words_gDD_post_rF_obj = {};

// fetchConceptGraph_gDD is called by GlobalDynamicDataMaintenance
// fetchConceptGraph_gDD obtains the full list of words from SQL;
// then calls identifyWordUpdates_gDD to determine all edits that must be made
export const cp_fetchConceptGraph_gDD = async (conceptGrapTableName,currentTaskNumber,updateChangedWords) => {
    words_gDD_pre_rF_obj = {};
    words_gDD_post_rF_obj = {};
    var sql = " SELECT * FROM "+conceptGrapTableName;
    // console.log("sql: "+sql);

    sendAsync(sql).then( async (words_gDD_pre_arr) => {
        var numWords = words_gDD_pre_arr.length;

        for (var w=0;w<numWords;w++) {
            var nextWord = words_gDD_pre_arr[w];
            ...
            words_gDD_pre_rF_obj[nextWord_slug]=JSON.parse(JSON.stringify(nextWord_rawFile_obj));
        }

        words_gDD_post_rF_obj = identifyWordUpdates_gDD(words_gDD_pre_rF_obj);
        ...
        for (var w=0;w<numWords;w++) {
            ...
            var word_original_obj = words_gDD_pre_rF_obj[nextWord_slug];
            var word_updated_obj = words_gDD_post_rF_obj[nextWord_slug];
        ...
        ControlPanelFunctions.updateControlPanelDisplay(numUpdatedWords_gDD,currentTaskNumber);
        ControlPanelFunctions.proceedWithNextTask();
        return numUpdatedWords_gDD;
    });
}
...
export const identifyWordUpdates_gDD = (words_in_obj) => {
    var words_out_obj = {};
    var schema_rels_arr = [];
